...
export class LabCollaborationStore {
  async create(payload: InsertLabCollaboration & { labName: string }) {
    ...
    const { data: inserted, error } = await supabase
      ...
      .select("id, lab_id, lab_name, contact_name, contact_email, preferred_contact, target_labs, collaboration_focus, resources_offered, desired_timeline, additional_notes, created_at")
      .single();
